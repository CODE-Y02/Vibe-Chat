...
export const registerSignalingHandlers = (
  ...
  socket.on("call-accepted", async ({ to }: { to: string }) => {
    console.log(
      `[Signaling] call-accepted: ${user.userId} accepted call from ${to}`,
    );
  ...
  socket.on("reject-call", async ({ to }: { to: string }) => {
    console.log(
      `[Signaling] reject-call: ${user.userId} rejected call from ${to}`,
    );
  ...
  socket.on("cancel-call", async ({ to }: { to: string }) => {
    console.log(
      `[Signaling] cancel-call: ${user.userId} cancelled call to ${to}`,
    );
    io.to(to).emit("call-cancelled", { from: user.userId });
  });

  socket.on("call-received", async ({ to }: { to: string }) => {
    console.log(
      `[Signaling] call-received acknowledgement: ${user.userId} received call from ${to}`,
    );
    io.to(to).emit("call-received", { from: user.userId });
  });
  ...
  socket.on(
    "iceCandidate",
    async ({ to, candidate }: { to: string; candidate: unknown }) => {
