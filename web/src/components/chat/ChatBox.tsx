      ...
      <div
        className={cn(
          ...
          isOwn
            ...
            : "bg-muted/70 dark:bg-muted/40 text-foreground rounded-tl-sm border border-border/50 backdrop-blur-md",
        )}
      >
        {message.text}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.1] dark:via-white/[0.05] to-transparent -translate-x-full animate-[shimmer_3s_infinite]" />
      </div>
    </motion.div>
  );
}
...
export const ChatBox = memo(
  ({ onReport, isMinimized = false, onToggleMinimize }: ChatBoxProps = {}) => {
    const { data: sessionData } = useSession();
    const [text, setText] = useState("");
    // Removed internal isMinimized state to support lifted state for parent layout sync
      ...
      if (session.isDirectCall) {
        try {
          ...
          queryClient.invalidateQueries({
            queryKey: ["messages", session.strangerId],
          });
        } catch (err) {
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-black text-[10px] md:text-xs uppercase tracking-[0.2em]">
                  Live Chat ({session.messages.length})
                </span>
              </div>
          ...
          >
            {!session.isMatched ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-8 md:p-12 space-y-6 md:space-y-8">
                ...
                <div>
                  <h3 className="text-lg md:text-xl font-black tracking-tight uppercase mb-2 md:mb-3 italic text-gradient px-4">
                    Private Vibe
                  </h3>
              ...
              <>
                <div className="p-4 md:p-6 border-b border-border bg-muted/20 flex items-center justify-between z-10">
                    ...
                    <div className="relative shrink-0">
                      ...
                      >
                        <Avatar className="w-10 h-10 md:w-12 md:h-12 border border-border shadow-lg">
                          <AvatarImage
                            src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${session.strangerId}`}
                          />
                    ...
                    <div className="min-w-0">
                      <h3 className="font-black text-[9px] md:text-[10px] tracking-[0.2em] uppercase text-foreground/80 truncate">
                        {session.isDirectCall
                          ? session.peerName || "Friend"
                          : "Stranger"}
                      </h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[8px] md:text-[9px] text-primary font-black uppercase tracking-widest">
                          Live Connection
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 md:gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        friendMutation.isPending || friendMutation.isSuccess
                      }
                      variant="ghost"
                      size="icon"
                      className={cn(
                    ...
                    <Button
                      ...
                      className="h-9 w-9 md:h-12 md:w-12 rounded-xl md:rounded-2xl hover:bg-muted text-muted-foreground/40 hover:text-foreground transition-all"
                    >
                  ...
                  <AnimatePresence initial={false}>
                    {session.messages.length === 0 ? (
                      ...
                      >
                        ...
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-center px-4">
                          Vibe Check Passed
                        </span>
                      </motion.div>
                    ) : (
                      session.messages.map((msg: Message) => (
                        <MessageBubble
                          ...
                          message={msg}
                          isOwn={
                            msg.senderId === (sessionData?.internalId || "me")
                          }
                        />
                      ))
                    )}
                  ...
                  <AnimatePresence>
                    {session.isPeerTyping && (
                      ...
                      >
                        ...
                        <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground animate-pulse leading-none">
                          Vibe Buddy Typing...
                        </span>
                      </motion.div>
                    )}
                ...
                <form
                  className="p-4 md:p-6 border-t border-border bg-muted/20 flex gap-3 md:gap-4 items-center z-10"
                  onSubmit={handleSend}
                >
                  <Input
                    value={text}
